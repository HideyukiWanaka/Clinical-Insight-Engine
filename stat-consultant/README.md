...
CORS is therefore **off by default**; neither mode needs it. Set
`STAT_CONSULTANT_DEV_CORS=1` only if you point a browser directly at :8000 from
another origin. (It stays off normally because `POST /api/rstudio/insert` is
deliberately unauthenticated, and permissive CORS would let any localhost page
push R code into the user's editor.)
