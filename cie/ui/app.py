...
def _get_services() -> dict:
    ...
    The actual DI assembly lives in :func:`cie.api.services.build_services`
    (Phase 1 / R1-1) so the FastAPI API layer and the Streamlit UI construct
    the exact same dependency graph.  This wrapper only adds Streamlit-side
    process caching.
    """
...
def _build_dataset_context(csv_bytes: bytes | None) -> dict:
    """Thin wrapper around :func:`cie.api.dataset.build_dataset_context`.

    Extracted in Phase 1 / R1-2 so the FastAPI ``/api/intent`` handler and the
    Streamlit UI derive column metadata identically. Behaviour is unchanged.
    """
    from cie.api.dataset import build_dataset_context

    return build_dataset_context(csv_bytes)
